...
class ReTradingEngine:
    ...
    def __init__(self):
        ...
        self.model_training_len = 1200
        self.model_testing_len = 400
        self.model_training_step = 100
        self.model_train_test_ratio = 0.8
    ...
    # ---------------------------------------------------------------------------------------------------
    def re_training_strategy_test(self, csv_path):
        self.set_initial_data_from_csv(csv_path)

        model_training_len = 100
        model_training_step = 100

        processed_data = self.process_timeseries_for_training_binary_trend(self.market_data, model_training_len, model_training_step)
        print("model_training_len: {}, model_training_step: {}   samples: {}".format(model_training_len, model_training_step, len(processed_data.index)))
        ...
        while subset_start_index + subset_length < processed_data_len:
            ...
            # print(processed_data_subset.info())

            # processed_data_subset = processed_data[processed_data.index >= subset_start_index]
            # # processed_data_subset.reset_index(inplace=True)
            # processed_data_subset = processed_data_subset[processed_data_subset.index < subset_length]
            # processed_data_subset.reset_index(inplace=True)

            # print("total samples: {}".format(len(processed_data_subset.index)))

            total_samples = len(processed_data_subset.index)
            train_count = total_samples - 2 #use the last item only for prediction
    ...
    # ---------------------------------------------------------------------------------------------------
    def process_timeseries_for_training_binary_trend(self, data, training_len, training_step):
        '''
        ...
        for index in range(0, limit, training_step):
            ...
            open_ask_price = data["askPrice"][index + training_len]
            open_bid_price = data["bidPrice"][index + training_len]
            spread = open_ask_price - open_bid_price

            # find the next profitable movement
            response = -1 # invalid by default
            j = index + training_len + 1
            while j < data_len:
                close_ask_price = data["askPrice"][j]
                close_bid_price = data["bidPrice"][j]

                if close_bid_price > open_ask_price:
                    #buy
                    response = 1
                    up_count += 1
                    break

                if close_ask_price < open_bid_price:
                    #sell
                    response = 0
                    down_count += 1
                    break
            ...
            # plt.plot(data.index[index + training_len:j], data["askPrice"][index + training_len:j], "--r", label="askPrice after")
            # plt.plot(data.index[index + training_len:j], data["bidPrice"][index + training_len:j], "-r", label="bidPrice after")
            # plt.figtext(.2, 0.8, response)
            ...
            processed_training_data["Response"].append(response)
    ...
    def run_re_training_strategy_on_processed_data(self, processed_data, subset_length):
        ...
        while subset_start_index + subset_length < processed_data_len:
            ...
            processed_data_subset = processed_data[subset_start_index:subset_start_index + subset_length]
            processed_data_subset.reset_index(inplace=True)
            # print ("subset time: {}".format(current_time_millis() - a_millis))
            # a_millis = current_time_millis()
            # print(processed_data_subset.info())
            ...
            # print("train test subset time: {}".format(current_time_millis() - a_millis))
            # a_millis = current_time_millis()
            ...
            # print("random forest fit time: {}".format(current_time_millis() - a_millis))
            # a_millis = current_time_millis()

            # ada_score = clf_ada.score(X_test.values, y_test)
            rf_score = clf_rf.score(X_test.values, y_test)

            # print("random forest score time: {}".format(current_time_millis() - a_millis))
            # a_millis = current_time_millis()
    ...
    def print_current_acuracy(self, train_test_ratio, training_step):
        ...
        score = self.get_strategy_acuracy_on_params(self.market_data, self.model_training_len, self.model_testing_len,
                                                    self.model_training_step, train_test_ratio)
    ...
    def optimize_model_params(self):
        ...
        for i in range(combinations_num):
            ...
            fitness = self.get_strategy_acuracy_on_params(self.market_data, training_len_p, testing_len_p,
                                                          training_step, train_test_ratio)
    ...
    def train(self):
        ...
        processed_data = self.process_timeseries_for_training(self.market_data, self.model_training_len,
                                                              self.model_testing_len, self.model_training_step)
    ...
    def test_model_on_csv(self, csv_path):
        ...
        processed_data = self.process_timeseries_for_training(test_market_data, self.model_training_len,
                                                              self.model_testing_len, self.model_training_step)
    ...
    def on_market_tick(self, timestamp, symbol, bid_price, bid_size, ask_price, ask_size):
        ...
        self.market_data.loc[last_index] = [last_index, timestamp, symbol, 0, bid_price, bid_size, ask_price, ask_size,
                                            mid_price, smart_price]
    ...
    def predict(self):
        ...
        prediction = self.model.predict(
            [[stddev, variance, linregress_obj.slope, linregress_obj.stderr, training_movement]])
        if len(prediction) == 0:
    ...
    def plot_prediction_history(self):
        ...
        print("correct: {}, total: {}, accuracy: {}".format(correct_predictions, total_predictions,
    ...
    def plot_market_data(self):
        plt.plot(self.market_data.index[0:self.initial_market_data_len],
                 self.market_data["bidPrice"][0:self.initial_market_data_len], "-b", label="initial bidPrice")
        plt.plot(self.market_data.index[0:self.initial_market_data_len],
                 self.market_data["askPrice"][0:self.initial_market_data_len], "--b", label="initial bidPrice")

        plt.plot(self.market_data.index[self.initial_market_data_len:],
                 self.market_data["bidPrice"][self.initial_market_data_len:], "-r", label="tracked bidPrice")
        plt.plot(self.market_data.index[self.initial_market_data_len:],
                 self.market_data["askPrice"][self.initial_market_data_len:], "--r", label="tracked bidPrice")
...
def test() :
    ...
    #engine.print_current_acuracy(0.92, 50)

    #engine.test_model_on_csv("C:/Projects/QuantRat/data/btc_test.csv")
